...
(Xtrain, ytrain), (Xtest, ytest), (dm_train, ) = train_test_split_reg(X, y, L, missing_class; ratio=ratio, seed=seed)
...
d = Dict(Symbol.(["ratio", "seed", "α", "missing_class", "model"]) .=> [ratio, seed, α, missing_class, full_model])
safesave(datadir("models", "missing", savename("model", d, "bson")), d)